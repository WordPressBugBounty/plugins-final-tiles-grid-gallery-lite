...
if (ftg_fs()->can_use_premium_code__premium_only())
{
    if(! function_exists('removeDisabled')) {
        function removeDisabled(&$arr) {
          for($i=0; $i<count($arr); $i++)
            $arr[$i] = str_ireplace("|disabled", "", $arr[$i]);
        }
    }
}
...
$this->addField("Advanced", "loadMethod", array(
    ...
    "type" => "select",
    "values" => array(
        "Loading method" => array(
            "sequential|Sequential",
            "lazy|Lazy (load images on scroll)"
        )
    ),
    "description" => "",
    "proCall" => false,
...
$this->addField("Advanced", "ajaxLoading", array(
    ...
    "values" => array(
        "Loading method" => array(
            "F|Complete markup on page",
            "lazy|Enable ajax loading"
        )
    ),
...
$this->addField("Links & Lightbox", "lightbox", array(
    ...
    "values" => array(
        "Link" => array(
            "none|No lightbox",
            "direct|Direct link to image (useful for external lightboxes)|disabled",
            "post|Post or WooCommerce product|disabled"
        ),
        "Lightboxes" => array(
            "lightbox2|Lightbox",
...
if (ftg_fs()->can_use_premium_code__premium_only())
{
  removeDisabled($this->fields["Links & Lightbox"]["fields"]["lightbox"]["values"]["Lightboxes"]);

  if ( ftg_fs()->is_plan_or_trial( 'ultimate' ) ) {
    $this->fields["Links & Lightbox"]["fields"]["lightbox"]["values"]["Link"] =
        array(
                " |No lightbox",
                "direct|Direct link to image (useful for external lightboxes)",
                "post|Post or WooCommerce product"
        );
    }
}
...
if (ftg_fs()->can_use_premium_code__premium_only())
{
    removeDisabled($this->fields["Links & Lightbox"]["fields"]["mobileLightbox"]["values"]["Lightboxes"]);

    if ( ftg_fs()->is_plan_or_trial( 'ultimate' ) ) {
        $this->fields["Links & Lightbox"]["fields"]["mobileLightbox"]["values"]["Link"] = array(
                " |No lightbox",
                "direct|Direct link to image (useful for external lightboxes)",
                "post|Post or WooCommerce product"
        );
...
if (ftg_fs()->can_use_premium_code__premium_only())
{
    ...
    $this->addField("Links & Lightbox", "lightboxOptions", array(
        "name" => "Lightbox options<br>(LightGallery and Magnific Popup)",
        "type" => "textarea",
        "description" => wp_kses( __("Custom options for chosen lightbox. Enter params without curly braces.<br>Example: <span style='font-family:monospace'>easing: linear, mode: 'lg-fade'</span>.<br>Documentation: <a href='http://sachinchoolur.github.io/lightGallery/docs/api.html' target='_blank'>LightGallery</a>, <a target='_blank' href='http://dimsemenov.com/plugins/magnific-popup/documentation.html'>Magnific Popup</a>.", "final-tiles-grid-gallery-lite"), array()),
        "proCall" => false,
        "excludeFrom" => array()
    ));


    $this->addField("Links & Lightbox", "lightboxOptionsMobile", array(
        "name" => "Mobile Lightbox options<br>(LightGallery and Magnific Popup)",
        "type" => "textarea",
        "description" => wp_kses( __("Custom options for chosen lightbox. Enter params without curly braces.<br>Example: <span style='font-family:monospace'>easing: linear, mode: 'lg-fade'</span>.<br>Documentation: <a href='http://sachinchoolur.github.io/lightGallery/docs/api.html' target='_blank'>LightGallery</a>, <a target='_blank' href='http://dimsemenov.com/plugins/magnific-popup/documentation.html'>Magnific Popup</a>.", "final-tiles-grid-gallery-lite"), array() ),
        "proCall" => false,
        "excludeFrom" => array()
...
{
    if ( ftg_fs()->is_plan_or_trial( 'ultimate' ) ) {
      $this->fields["Captions"]["fields"]["captionBehavior"]["values"]["Effect"] = array(
        "never|Never show captions",
        "none|Fade in",
        "fixed|Fixed",
        "fixed-bg|Fixed with background",
        "fixed-then-hidden|Fixed, hidden on mouse hover",
        "fixed-bottom|Fixed at bottom",
...
{
    if ( ftg_fs()->is_plan_or_trial( 'ultimate' ) ) {
      $this->fields["Captions"]["fields"]["captionMobileBehavior"]["values"]["Behavior"] = array(
        "desktop|Same as desktop",
        "none|Never show captions",
        "fixed-bg|Fixed with background",
        "fixed-bottom|Fixed at bottom",
        "fixed-then-hidden|Visible, hidden on touch"
      );
    }
}
if(ftg_fs()->can_use_premium_code__premium_only())
{
    $this->addField("Captions", "captionCustomFields", array(
        "name" => esc_html__("Custom fields","final-tiles-grid-gallery-lite"),
        "type" => "textarea",
        "description" => esc_html__("Insert 1 custom field on each line. Add an optional label after the custom field code.", "final-tiles-grid-gallery-lite"),
        "values" => array(),
        "proCall" => false,
...
$this->addField("Captions", "captionEmpty", array(
    ...
    "values" => array(
        "Empty captions" => array(
            ...
            "show|Show empty captions|disabled"
        )
    ),
    "proCall" => false,
    "excludeFrom" => array()
...
{
    if ( ftg_fs()->is_plan_or_trial( 'ultimate' ) ) {
      $this->fields["Captions"]["captionEmpty"]["values"]["Empty captions"] =  array(
            "hide|Don't show empty captions",
...
if (ftg_fs()->can_use_premium_code__premium_only())
{
  removeDisabled($this->fields["Captions"]["fields"]["captionEasing"]["values"]["Easing"]);
}
...
$this->addField("Captions", "captionVerticalAlignment", array(
    ...
    "values" => array(
        "Caption vertical alignment" => array(
            "top|Top",
            "middle|Middle",
            "bottom|Bottom"
        )
...
$this->addField("Captions", "captionHorizontalAlignment", array(
    ...
    "values" => array(
        "Caption horizontal alignment" => array(
            "left|Left",
            "center|Center",
            "right|Right"
        )
