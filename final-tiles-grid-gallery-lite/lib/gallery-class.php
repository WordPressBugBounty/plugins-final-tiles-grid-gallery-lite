...
if (!class_exists("FinalTilesGallery"))
{
	class FinalTilesGallery
	{
		...
		public function __construct($galleryId, $db, $defaultValues, $attrs)
		{
			$this->id = $galleryId;
		...
		public function add_gallery_js(){
			...
			$lightbox = wp_is_mobile() ?
				$gallery->mobileLightbox == "desktop" ? $gallery->lightbox : $gallery->mobileLightbox
				:
				$gallery->lightbox;
		...
		public function getFilters()
		{
			if(ftg_fs()->is__premium_only())
			{
				...
				{
					if(empty($this->gallery->filters))
						return "";

					$css = array();
					foreach (explode("|", $this->gallery->filters) as $f) {
		...
		{
			return  $this->gallery->enableFacebook == "T" ||
					$this->gallery->enableTwitter == "T" ||
					$this->gallery->enablePinterest == "T";
		}
		...
		public function render()
		{
			...
			if(ftg_fs()->is__premium_only())
			{
				if(ftg_fs()->is_plan_or_trial('ultimate'))
				{
					$html .= "#ftg-".absint($this->id.$rid)." .tile.ftg-preload { transform: scaleY(" .(absint($gallery->loadedScaleY)/100) .") scaleX(" .(absint($gallery->loadedScaleX)/100) .") translate(" . (int)$gallery->loadedHSlide . 'px,' . (int)$gallery->loadedVSlide . "px) rotateX(" . (int)$gallery->loadedRotateX .  "deg) rotateY(" . (int)$gallery->loadedRotateY .  "deg); }\n";
					$html .= "#ftg-".absint($this->id.$rid)." .tile.ftg-preload { transition: all " . esc_attr($loadedEasing) .' '.(absint($gallery->loadedDuration/1000) ."s, opacity linear " .(absint($gallery->loadedDuration))/1000) ."s; }\n";
					$html .= "#ftg-".absint($this->id.$rid)." .tile img.item { transition-duration: " .(absint($gallery->hoverDuration)/1000) ."s; }\n";
				}
			}
			...
			if($gallery->hoverZoom != 100 || $gallery->hoverRotation != 0)
			{
				...
				foreach($this->cssPrefixes as $prefix)
				{
					$html .= "\t" . $prefix."transform: "
					         . $this->cssRuleRotation($prefix)
					         . $this->cssRuleZoom($prefix)
					         .";\n";
				}
			...
			if(strlen($gallery->filters))
			{
				if(ftg_fs()->is__premium_only())
				{
					if(ftg_fs()->is_plan_or_trial("ultimate"))
					{
						...
						$filter_url = $gallery->filterClick == 'F' ? '#ftg-set-ftgall' : '?ftg-set=all#' . absint( $this->id );
						$html       .= "<div class='ftg-filters'>\n";
						$html       .= "\t<a data-filter='all' href='" . esc_url( $filter_url ) . "' class='selected'>" . esc_html( $gallery->allFilterLabel ) . "</a>\n";
						foreach($filters as $filter)
						{
							$filter_url = $gallery->filterClick == 'F' ? '#ftg-set-' . FinalTilesGallery::slugify($filter) : '?ftg-set=' . FinalTilesGallery::slugify($filter) . '#'. absint($this->id);

							$html .= "\t<a data-filter='". $this->slugify($filter) ."' href='".esc_url($filter_url)."'>".esc_html($filter)."</a>\n";
						}
						$html .= "</div>\n";
					}
				}
		...
		public function images_markup()
		{
			...
			$lightbox = wp_is_mobile() ?
				$gallery->mobileLightbox == "desktop" ? esc_attr($gallery->lightbox): esc_attr($gallery->mobileLightbox)
				:
				esc_attr($gallery->lightbox);
			...
			foreach($this->images as $image)
			{
				...
				else
				{
					...
					$html .= ($lightbox == "lightbox2" &&
											empty($image->link) ?
												"data-lightbox='".esc_attr($rel)."'" : "");
    ...
    //          print_r($posts);
		...
		public function getPosts()
		{
			...
			if(ftg_fs()->is_plan_or_trial__premium_only("ultimate"))
			{
				...
				if (isset($this->gallery->post_taxonomies) && !empty($this->gallery->post_taxonomies))
				{
					$taxonomies['relation'] = 'OR';
					if(isset($this->gallery->taxonomyOperator)){
						$taxonomies['relation'] = $this->gallery->taxonomyOperator;
					}

					foreach(explode(",", $this->gallery->post_taxonomies) as $item)
					{
						$t = explode("|", $item);
						$taxonomies[] = 	array(
							'taxonomy' => $t[0],
					...
					$posts_args['tax_query'] = $taxonomies;
				}

				if($this->gallery->post_types) {

					$fmt_types = array();
					foreach(explode(",", $this->gallery->post_types) as $type){
						$fmt_types []= "\"" . $type . "\"";
					}

					$posts_args['post_type'] = $fmt_types;
				}

				if(isset($this->gallery->post_categories) && !empty($this->gallery->post_categories)) {

					$cat_id = array();
					foreach(explode(",", $this->gallery->post_categories) as $id) {
						$cat_id []= intval($id);
					}
					$posts_args['category'] = $cat_id;
				}

				if(! $noPaging) {
					$skip = ($page - 1) * $postsPerPage;
					$posts_args['posts_per_page'] = $postsPerPage;
					$posts_args['offset'] = $skip;
				}
				...
				foreach ($posts as &$post)
				{
					...
					if($post_thumbnail_id)
					{
						...
						if(isset($this->gallery->taxonomyAsFilter) && ! empty($this->gallery->taxonomyAsFilter))
						{
							$terms = get_the_terms($post->ID, $this->gallery->taxonomyAsFilter);
							$filters = array();
							foreach($terms as $t)
							{
								if(isset($t->slug))
							...
							case "custom":
								...
								foreach($fields as $field)
								{
									...
									//if(! empty($field))
									{
										...
										$item->description .= "<span class='ftg-meta ftg-meta-". esc_attr( $css_class ) ."'>" . $field . "</span>";
									}
								}
								break;
						}
