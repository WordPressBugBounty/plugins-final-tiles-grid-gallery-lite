...
if(! class_exists('FinalTiles_Gallery'))
{
    class FinalTiles_Gallery
    {
		...
		public function register_gutenberg_block()
		{
			...
			wp_register_script(
				'ftg-gallery-block',
				plugins_url( 'scripts/gutenberg_block.js', __FILE__ ),
				array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wp-components' )
			);

			// Editor CSS
			wp_register_style(
				'ftg-gallery-block-editor',
				plugins_url( 'admin/css/gutenberg_block.css', __FILE__ ),
        ...
        public function review()
        {
            ...
            <div class="notice notice-info is-dismissible ftg-review-notice">
                <p><?php esc_html_e( 'Hey, I noticed you created a photo gallery with Final Tiles - that’s awesome! Would you mind give it a 5-star rating on WordPress to help us spread the word and boost our motivation for new featrues?', 'final-tiles-grid-gallery-lite' ); ?></p>
                <p><strong><?php esc_html_e( 'MachoThemes', 'final-tiles-grid-gallery-lite' ); ?></strong></p>
                <p>
                    <a href="https://wordpress.org/support/plugin/final-tiles-grid-gallery-lite/reviews/?filter=5#new-post" class="ftg-dismiss-review-notice ftg-review-out" target="_blank" rel="noopener"><?php esc_html_e( 'Ok, you deserve it', 'final-tiles-grid-gallery-lite' ); ?></a><br>
                    <a href="#" class="ftg-dismiss-review-notice" rel="noopener"><?php esc_html_e( 'Nope, maybe later', 'final-tiles-grid-gallery-lite' ); ?></a><br>
                    <a href="#" class="ftg-dismiss-review-notice" rel="noopener"><?php esc_html_e( 'I already did', 'final-tiles-grid-gallery-lite' ); ?></a><br>
                </p>
            </div>
        ...
        public function update_configuration()
        {
            if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery'))
            {
                ...
                $config = (isset($_POST['config']) ? wp_unslash( $_POST['config'] ) : ''); // phpcs:ignore
        ...
        public function get_configuration()
        {
            if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery'))
            {
                ...
                echo stripslashes( $gallery ); // phpcs:ignore
            }
            exit();
        }
        ...
        public function add_gallery_scripts()
        {
            ...
            wp_register_script('finalTilesGallery', plugins_url('scripts/jquery.finalTilesGallery.js', __FILE__), array('jquery'), FTGVERSION, true);
            wp_enqueue_script('finalTilesGallery');


            wp_register_style('finalTilesGallery_stylesheet', plugins_url('scripts/ftg.css', __FILE__), array(), FTGVERSION);
            wp_enqueue_style('finalTilesGallery_stylesheet');

            if ( ftg_fs()->is__premium_only() ) {
                wp_register_script('magnific_script', plugins_url('lightbox-pro/magnific/script.js', __FILE__), array('jquery'), FTGVERSION, true);
                wp_register_script('prettyphoto_script', plugins_url('lightbox-pro/prettyphoto/script.js', __FILE__), array('jquery'), FTGVERSION, true);
                wp_register_script('colorbox_script', plugins_url('lightbox-pro/colorbox/script.js', __FILE__), array('jquery'), FTGVERSION, true);
                wp_register_script('fancybox_script', plugins_url('lightbox-pro/fancybox/script.js', __FILE__), array('jquery'), FTGVERSION, true);
                wp_register_script('swipebox_script', plugins_url('lightbox-pro/swipebox/script.js', __FILE__), array('jquery'), FTGVERSION, true);
                wp_register_script('image_lightbox_script', plugins_url('lightbox-pro/image-lightbox/js/script.js', __FILE__), array('jquery'), FTGVERSION, true);
                wp_register_script('lightgallery_script', plugins_url('lightbox-pro/lightgallery/script.js', __FILE__), array('jquery'));
            }
            wp_register_script('lightbox2_script', plugins_url('lightbox/lightbox2/js/script.js', __FILE__), array('jquery'));

            if ( ftg_fs()->is__premium_only() ) {
                wp_register_style('magnific_stylesheet', plugins_url('lightbox-pro/magnific/style.css', __FILE__));
                wp_register_style('prettyphoto_stylesheet', plugins_url('lightbox-pro/prettyphoto/style.css', __FILE__));
                wp_register_style('colorbox_stylesheet', plugins_url('lightbox-pro/colorbox/style.css', __FILE__));
                wp_register_style('fancybox_stylesheet', plugins_url('lightbox-pro/fancybox/style.css', __FILE__));
                wp_register_style('swipebox_stylesheet', plugins_url('lightbox-pro/swipebox/style.css', __FILE__));
                wp_register_style('lightgallery_stylesheet', plugins_url('lightbox-pro/lightgallery/css/lightgallery.min.css', __FILE__));
            }
        ...
        public function gallery_admin_init()
        {
            ...
            wp_register_script('final-tiles-gallery', plugins_url('admin/scripts/final-tiles-gallery-admin.js', __FILE__), array('jquery', 'media-upload', 'thickbox', 'materialize'));
        ...
        {
            $overview = add_menu_page('Final Tiles Gallery', 'Final Tiles Gallery', 'edit_posts', 'ftg-lite-gallery-admin', array($this, 'add_overview'), plugins_url('admin/icon.png', __FILE__));
            $add_gallery = add_submenu_page('ftg-lite-gallery-admin', esc_html__('FinalTiles Gallery >> Add Gallery', 'final-tiles-grid-gallery-lite'), esc_html__('Add Gallery', 'final-tiles-grid-gallery-lite'), 'edit_posts', 'ftg-add-gallery', array($this, 'add_gallery'));
        ...
        public function assign_filters()
        {
            if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery')) {
                ...
                if (isset($_POST['source']) && $_POST['source'] == 'posts') {
                    ...
                    foreach (explode(",", $ids) as $id) {
                        $result = $this->FinalTilesdb->editImage( absint( $id ), array("filters" => sanitize_text_field($filters)));
                    }
                }
            }
            wp_die();
	    ...
	    public function toggle_visibility()
	    {
		    if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery'))
		    {
		        ...
		        foreach (explode(",", $ids) as $id)
		        {
				    ...
				    $this->FinalTilesdb->editImage( absint( $id ), array("hidden" => $image->hidden == 'T' ? 'F' : 'T'));
			    }
		    }
		    wp_die();
	    }
	    ...
	    public function assign_group()
	    {
		    if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery')) {
			    ...
			    if (isset($_POST['source']) && $_POST['source'] == 'posts') {
				    ...
				    foreach (explode(",", $ids) as $id) {
					    $result = $this->FinalTilesdb->editImage($id, array("group" =>sanitize_text_field($group)));
				    }
			    }
		    }
		    wp_die();
        ...
        public function add_image()
        {
            if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery')) {
                ...
                $enc_images = (isset($_POST['enc_images']) ? wp_unslash( $_POST['enc_images'] ) : ''); // phpcs:ignore
        ...
        public function load_chunk()
        {
            ...
            if (check_admin_referer('finaltilesgallery', 'finaltilesgallery'))
            {
                ...
                echo $FinalTilesGallery->images_markup(); // phpcs:ignore
            }
            wp_die();
        }
        ...
        public function refresh_gallery()
        {
            if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery')) {
                ...
                if(ftg_fs()->is__premium_only())
                {
                    ...
                    {
                        if (isset($_POST['source']) && sanitize_text_field( wp_unslash( $_POST['source'] ) ) == 'posts')
                            $this->list_posts__premium_only(null);
                        if (isset($_POST['source']) && sanitize_text_field( wp_unslash( $_POST['source'] ) ) == 'woocommerce')
                            $this->list_posts__premium_only('product');
                    }
                }
            }
        }

        public function list_posts__premium_only($post_type)
        {
            if ( check_admin_referer( 'FinalTiles_gallery', 'FinalTiles_gallery' ) ) {
                $args = array(
                    'order'               => 'DESC',
                    ...
                    'nopaging'            => ((isset($_POST['ftg_max_posts']) && intval( $_POST['ftg_max_posts'] ) == 0) ? true : false ),
                    'posts_per_page'      => (isset($_POST['ftg_max_posts']) ? intval( $_POST['ftg_max_posts'] ): 0 ),
                    'meta_query'          => array( array(
                    'key' => '_thumbnail_id',
                ) ),
                );
                $args['tax_query'] = array(
                    "relation" => (isset($_POST['ftg_taxonomyOperator']) ? sanitize_text_field( wp_unslash( $_POST['ftg_taxonomyOperator'] ) ): 'OR' ),
                );
                if ( $post_type != null ) {
                    $args['post_type'] = $post_type;
                }
                if ( isset( $_POST['post_types'] ) ) {
                    $args['post_type'] = explode( ",", sanitize_text_field( wp_unslash( $_POST['post_types'] ) ) );
                }
                if ( isset( $_POST['post_taxonomy'] ) && ! empty( $_POST['post_taxonomy'] )) {
                    foreach ( explode( ",", sanitize_text_field( wp_unslash( $_POST['post_taxonomy'] ) ) ) as $taxonomy_raw ) {
                        $t = explode( "|", $taxonomy_raw );
                        $args['tax_query'][] = array( array(
                            'taxonomy' => sanitize_text_field($t[0]),
                            'field'    => 'term_id',
                            'terms'    => intval($t[1]),
                        ) );
                    }
                ...
                if ( isset( $_POST['woo_categories'] ) ) {
                    $args['tax_query'][] = array( array(
                        'taxonomy' => 'product_cat',
                        'field'    => 'id',
                        'terms'    => explode( ",", sanitize_text_field( wp_unslash($_POST['woo_categories'] ) ) ),
                    ) );
                }
        ...
        public function save_gallery()
        {
            if (check_admin_referer('FinalTiles_gallery', 'FinalTiles_gallery')) {
                ...
                $data = array(
                    ...
                    // loadedRotate doesn't seem to exist
	    ...
	    public function create_gallery( $attrs ) {
		    ...
		    if ( class_exists( 'FinalTilesGallery' ) ) {
			    $FinalTilesGallery = new FinalTilesGallery( $galleryId, $this->FinalTilesdb, $this->defaultValues, $attrs );
        ...
        public static function getFieldType($field)
        {
            if ( ftg_fs()->is__premium_only() )
            {
                if(ftg_fs()->is_plan_or_trial('basic'))
                {
                    if($field == "custom_isf")
                        return "custom_isf";
                }
                if(ftg_fs()->is_plan_or_trial('ultimate'))
                {
                    if($field == "filter")
                        return "filter";
                    if($field == "mobileLightbox")
                      return "select";
                    if($field == "customCaptionIcon" ||
                      $field == "hoverZoom")
                        return "text";
                    if($field == "hoverDuration")
                      return "slider";
                    if($field == "hoverRotation")
                      return "slider";
                }
            }
